...
LIST(Groceries[1:10])  #another view

#Find Frequent Itemset
frequentItems = eclat (Groceries, parameter = list(supp = 0.005, minlen= 3, maxlen = 5)) 
...
rules3 = apriori (data=Groceries, parameter=list (supp=0.002,conf = 0.7), appearance = list (default="lhs",rhs="whole milk"))
inspect(rules3[1:15])
inspect(rules3)

#Find out what events were influenced by a given event
subset1 = apriori(data=Groceries, appearance=list(default='lhs',rhs='whole milk'))
subset1 = subset(rules2, subset=rhs %in% 'bottled beer')
inspect(subset1)
inspect(rules2)
subset2 = subset(rules2, subset=lhs %ain% c('baking powder','soda') ) #both the items should be in lhs
inspect(subset2)
subset2a = subset(rules2, subset=lhs %in% c('baking powder','soda') )#either of the items should be in lhs
inspect(subset2a)
